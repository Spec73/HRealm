...
function buildResultItems(filtered, container) {
    container.innerHTML = '';
...
function initSearch() {
    ...
    if (searchInput && resultsDiv) {
        searchInput.addEventListener('input', (e) => {
            ...
            buildResultItems(filtered, resultsDiv);
        });
    ...
    // --- Mobile menu search ---
    const mobileInput = document.querySelector('.mobile-menu-search input');
    if (mobileInput) {
        // Create a results container inside the mobile menu, right after the search box
        ...
        if (!mobileResults) {
            ...
            mobileResults.style.cssText = 'display:none; margin-bottom:16px;';
            mobileInput.closest('.mobile-menu-search').insertAdjacentElement('afterend', mobileResults);
        }

        mobileInput.addEventListener('input', (e) => {
            const query = e.target.value.toLowerCase().trim();
            if (!query) { mobileResults.style.display = 'none'; return; }
            const filtered = landmarkData.filter(l =>
                l.name.toLowerCase().includes(query) || l.loc.toLowerCase().includes(query)
            );
            buildResultItems(filtered, mobileResults);
        });
    }
}
