...
function initGrid() {
    ...
    landmarkData.forEach(item => {
        ...
        card.innerHTML = `
            <img src="${item.imgs[0]}" alt="${item.name}" style="width:100%; height:250px; object-fit:cover; display:block;">
            <div class="card-info">
                <h3>${item.name}</h3>
                <p>${item.loc}</p>
                <button class="open-history" onclick="showHistory('${item.wiki}', '${item.name}', '${item.loc}')">Կարդալ պատմությունը</button>
            </div>
        `;
