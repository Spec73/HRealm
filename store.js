...
const items = [
    ...
    { name: "Himeji Castle Figure", price: 58, img: "https://i.ebayimg.com/images/g/1REAAOSw3NxiJWbY/s-l1600.webp", desc: "Զարդարեք ձեր սենյակը Շաչիհոկոյի կերպարով։ Շաչիհոկոն երևակայական արարած է՝ վագրի գլխով և կարպի մարմնով։ Այն ճապոնական ավանդական կերպար է, որը հաճախ կարելի է գտնել ճապոնական ամրոցների գագաթներինՀիմեջիի ավանդական ճապոնական ոճով թանաքով նկարչական տպագրություն բալենու ծաղիկների տակ" },
    ...
    { name: "Sydney Opera House Wallpaper", price: 40, img: "https://i.ebayimg.com/images/g/x0sAAOSwhLth6-9C/s-l1600.webp", desc: "Սիդնեյի օպերայի թատրոնի 3D պաստառ՝ հանվող, ինքնակպչուն, 1832թ." },
    ...
    { name: "Westminster Abbey House", price: 40, img: "https://i.ebayimg.com/images/g/qncAAeSwiJ5p3Cdx/s-l1600.webp", desc: "Գերազանց վիճակում գտնվող Վեստմինստերյան աբբայության 56-րդ բաժանմունքի Սուրբ Ծննդյան տուն" },
...
function openCart() {
    ...
    document.getElementById('cart-sum').innerHTML = `<h3 style="text-align:right;margin-top:20px;color:#2d1b18;">Ընդհանուր: $${total.toFixed(2)}</h3>`;
        ...
        if (user) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    localStorage.setItem('hrealm_user', JSON.stringify(user));
                }
            } catch {
                if (!user.orders) user.orders = [];
                user.orders.push(order);
